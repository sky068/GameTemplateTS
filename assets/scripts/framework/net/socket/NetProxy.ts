/**
 * Created by xujiawei on 2020-07-08 16:53:14
 */
...
import * as GameProtocols from './GameProtocals';
...
export class NetProxy {
    network: any = null;
    private _cachePushCallback: any[] = [];
    ...
    onNetworkOpen() {
        cc.director.emit(ConstData.StaticKey.EVENT_NETWORK_OPENED);
    }

    onNetworkClose() {
        cc.director.emit(ConstData.StaticKey.EVENT_NETWORK_CLOSED);
    }
    ...
    registerPush(key, cb, target) {
        ...
        let pushCallback = function (resp) {
            ...
            cc.director.emit(resp.act, resp);
        };
    ...
    login(origin, token) {
        ...
        let callback = function (resp) {
            if (resp.err != 0) {
                cc.director.emit(ConstData.StaticKey.EVENT_LOGIN_FAILED, resp);
                return;
            }
            ...
            //登录
            cc.director.emit(ConstData.StaticKey.EVENT_LOGIN_SUC, resp);
        };
        this.network.sendRequest(req, callback);
    ...
    logout() {
        ...
        //     cc.director.emit(farm.game.gmConst.SP_EVENT_LOGOUT_SUCCESS);
        // });
    }
    ...
    getRank(rankType) {
        ...
        //         cc.director.emit(farm.game.gmConst.SP_EVENT_GET_RANK_FAILED, resp);
        //         return;
        //     }
        ...
        //     }
        //     cc.director.emit(farm.game.gmConst.SP_EVENT_GET_RANK_SUCCESS, resp);
        // };
        // this.network.sendRequest(req, callback);
    }
    ...
    pushChat(resp) {
        // cc.director.emit(ConstData.StaticKey.EVENT_CHAT, resp);
    }

    pushExitRoom(resp) {
        // cc.director.emit(ConstData.StaticKey.EVENT_EXITROOM, resp);
    }


    pushPlayChess(resp) {
        // cc.director.emit(ConstData.StaticKey.EVENT_PLAYCHESS, resp);
    }
